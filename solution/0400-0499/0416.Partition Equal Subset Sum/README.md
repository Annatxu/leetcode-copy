...
2. 如果总和是奇数，返回 false（因为不能将其平均分成两个子集）。

3. 定义目标和 target = totalSum / 2。

4. 初始化一个大小为 target + 1 的布尔数组 dp，其中 dp[0] = true，因为和为 0 总是可以通过不选择任何元素来实现。

5. 对于输入数组中的每个数，从右向左更新 dp 数组（这样可以避免重复使用同一个数）。

6. 返回 dp[target] 的值，表示是否可以找到一个子集使其和为目标值。
